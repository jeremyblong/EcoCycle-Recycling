...
class SecurePinScreen extends Component {
    ...
    calculateToShow = () => {
        if (this.state.activityLoadingActive === true) {
            return (
                <Fragment>
                    <ActivityIndicator color={Colors.secondaryColor} style={{ marginTop: Sizes.fixPadding * 3.0 }} />
                </Fragment>
            );
        }
    }
            ...
            <View style={{ flexDirection: 'row', justifyContent: 'center', marginTop: Sizes.fixPadding * 7.0 }}>
                <View style={{
                    backgroundColor:
                        ...
                        this.state.pinCount == 6 
                        && this.state.pinCount != 0 ? Colors.secondaryColor : Colors.whiteColor,
                    borderColor:
                    this.state.pinCount == 1 ||
                        this.state.pinCount == 2 ||
                        this.state.pinCount == 3 ||
                        this.state.pinCount == 4 || 
                        this.state.pinCount == 5 || 
                        this.state.pinCount == 6 ? Colors.secondaryColor : 'gray',
    ...
    keyBoardDesign() {
        ...
        const renderItem = ({ item, index }) => (
            <TouchableOpacity
                ...
                {Number(item.id) === 12 ?
                    item.data : Number(item.id) === 10 ? item.data : <Text style={{ ...Fonts.black17SemiBold, color: "#fff" }}>{item.data}</Text>
                }
            </TouchableOpacity>
        )
        return (
...
const enterPinInfo = () => {
    return (
        <View style={{ alignItems: 'center' }}>
            <Text style={{ ...Fonts.white16SemiBold }}>Enter a <Text style={{ color: "blue", textDecorationLine: "underline", fontWeight: "bold" }}>unique</Text> PIN for your account</Text>
            <Text style={{ ...Fonts.gray15Medium, marginTop: Sizes.fixPadding - 3.0, textAlign: "center", margin: 12.5, color: "#fff" }}>
