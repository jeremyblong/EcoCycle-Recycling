...
router.get("/", (req, resppppp) => {
    ...
    collection.createIndex({ "mainData.newlyConstructedCoordsRandomizedNearby": "2dsphere" });

    const milesConvertedToMeters = (1609 * 2.125); // 1609 === meters in mile

    collection.find({ "mainData.newlyConstructedCoordsRandomizedNearby": {
        $near: {
            $geometry: {
                type: "Point",
        ...
        resppppp.json({
            message: "Successfully gathered related items based on geo-location!",
            listings: results
        })
    }).catch((err) => {
