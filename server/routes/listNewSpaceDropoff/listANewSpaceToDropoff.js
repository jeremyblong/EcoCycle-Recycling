...
router.post("/", async (req, resppppp, next) => {
    ...
    const dropCollection = Connection.db.db("test").collection("dropoffs");
    ...
    const foundMatch = await dropCollection.findOne({ postedByID: signedinUserID });
    ...
    if (foundMatch === null) {
        ...
        resppppp.json({
            message: "You can ONLY submit ONE space for availablity!",
            err: null
        })
    }
});
